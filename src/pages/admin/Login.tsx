...
export default function Login() {
  ...
  const ADMIN_EMAIL = 'arijitray.ray@gmail.com';
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (email !== ADMIN_EMAIL) {
      setError('Only authorized admin can create an account.');
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (data.user) {
        ...
        setActiveTab('login');
        setShowSignup(false); // Hide signup after successful creation
      }
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-md">
            ...
            {showSignup && (
              <TabsContent value="signup">
                <CardHeader>
                  ...
                  <CardDescription>
                    Set up the admin account (Only authorized admin email allowed)
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <form onSubmit={handleSignUp} className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Only {ADMIN_EMAIL} is allowed to create an admin account
                      </p>
                    </div>
        ...
        <div className="text-center mt-8 text-sm text-muted-foreground animate-fade-in">
          <p>{showSignup ? 'Create your admin account or sign in if you already have one' : 'Sign in to access the admin dashboard'}</p>
        </div>
      </div>
    </div>
  );
